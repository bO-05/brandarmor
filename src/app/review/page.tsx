...
export default function ReviewPage() {
  ...
  const [lastUpdated, setLastUpdated] = useState<{ listingId: string; status: ReviewStatus; title?: string | null } | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      setScoreCount(scoresResult.data.length);
      setError([decisionsResult.error, listingsResult.error, scoresResult.error].filter(Boolean).join(" ") || null);
  ...
  const statusIcon = (s: ReviewStatus) => {
    switch (s) { case "confirmed_counterfeit": return <XCircle className="w-4 h-4 text-destructive" />; case "rejected_legitimate": return <CheckCircle className="w-4 h-4 text-success" />; case "needs_more_evidence": return <HelpCircle className="w-4 h-4 text-warning" />; case "escalated": return <AlertTriangle className="w-4 h-4 text-destructive" />; default: return <AlertTriangle className="w-4 h-4 text-warning" />; }
  };

  const queueSummary = summarizeReviewQueue(items.map((item) => item.status));

  if (loading) return <div className="max-w-5xl mx-auto"><h1 className="text-2xl font-bold mb-6">Review Queue</h1><p>Loading internal review items...</p></div>;
        ...
        <div className="surface-card rounded-lg p-12 text-center">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            <Link href={emptyState.primaryHref} className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-md text-sm">
              <PlayCircle className="w-4 h-4" /> {emptyState.primaryLabel}
            </Link>
            {emptyState.secondaryActions.map((action) => (
              <Link key={action.href + action.label} href={action.href} className="inline-flex items-center gap-2 px-4 py-2 bg-secondary text-secondary-foreground rounded-md text-sm">
                {action.href.includes("import") ? <Upload className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                {action.label}
              </Link>
            ))}
          </div>
                  ...
                  {item.score?.reasons && item.score.reasons.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {item.score.reasons.map((r, i) => (
                        <p key={i} className="text-xs text-muted-foreground">{r.ruleName}: {r.message}</p>
                      ))}
                    </div>
                  )}
                  <ReviewDecisionPanel
                    ...
                    onSaved={async (status) => {
                      ...
                      setLastUpdated({ listingId: item.listingId, status, title: item.listing?.title });
                      await load();
                    }}
                  />
                </div>
