      ...
      <div className="surface-card rounded-lg p-6">
        ...
        {errors.length > 0 && (
          <div className="mt-4 p-3 bg-destructive/10 border border-destructive/30 rounded-md">
            ...
            {errors.map((e) => (
              <p key={`${e.line}-${e.field}-${e.message}`} className="text-xs text-destructive">Line {e.line}: {e.field} - {e.message}</p>
            ))}
          </div>
        )}

        <button type="button" onClick={handleImport} disabled={loading || !jsonInput.trim() || Boolean(syntaxError)}
