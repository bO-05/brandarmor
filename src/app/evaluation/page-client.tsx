...
type EvaluationAction =
  | { type: "load_started" }
  | { type: "load_finished"; data: EvaluationData; listingCount: number; reviewDecisionCount: number; error: string | null }
  | { type: "toggle_technical_table" };

const initialEvaluationState: EvaluationState = {
  data: null,
  loading: true,
  error: null,
  showTechnicalTable: false,
  listingCount: 0,
  ...
  switch (action.type) {
    ...
    case "load_finished":
      return {
        ...
        reviewDecisionCount: action.reviewDecisionCount,
        error: action.error,
        loading: false,
      };
    case "toggle_technical_table":
      return { ...state, showTechnicalTable: !state.showTechnicalTable };
  }
}
  ...
  async function load(compute: boolean = true) {
    ...
    try {
      ...
      dispatch({
        ...
        reviewDecisionCount: reviewResult.data.length,
        error: [result.error, listingsResult.error, reviewResult.error].filter(Boolean).join(" ") || null,
      });
    } catch (e) {
      dispatch({
        type: "load_finished",
        data: { cases: 0, metrics: [] },
        listingCount: 0,
        reviewDecisionCount: 0,
