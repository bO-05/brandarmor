...
Verification:
`npm run typecheck`, `BPOM_DISABLE_API=1 npm test` with 202/202 passing, `npm run build`, React Doctor v0.8.1 at 100/100, local health endpoint smoke checks, PDF `%PDF` verification, and five staged demo/report runs under 1.4 seconds using the labeled mock-provider path.
...
Verification:
`npm run typecheck`, `BPOM_DISABLE_API=1 npm test` with 202/202 passing, `npm run build`, React Doctor v0.8.1 at 100/100, plus local HTTP checks for root favicon, seeded Brands, blank listing rejection, demo core, and judge completion.
